...
void fixModelIndices(tree *tr, int endsite, boolean fixRates)
{
    ...
    int model1 = tr->model[0];
    for (int i = 1; i < endsite; ++i) {
        if(tr->model[i] != model1) {
            tr->partitionData[model1].upper = i;
            tr->partitionData[model1 + 1].lower = i;
            model1 = tr->model[i];
        }
    }
    ...
    for(int model = 0; model < tr->NumberOfModels; model++)
    {
        const int lower =  tr->partitionData[model].lower;
...
static void copyFiles(FILE *source, FILE *destination)
{
    ...
    while(!feof(source))    {
        if(ferror(source) != 0)
            assert(0);
        const size_t c = fread(buf, sizeof(char), 1024, source);
        if(c > 0)
...
static void concatenateBSFiles(int processes, char fileName[1024])
{
    if(processID == 0)
    {
        FILE *destination = myfopen(fileName, "w"),
                *source = (FILE*)NULL;
...
{
    int n, bestIndex, bootstrapsPerformed;
    ...
    for(i = 0; i < n && !bootStopIt; i++) {
        ...
        if(i % 10 == 0)
        {
            ...
            {
                FILE *f2 = myfopen(tree_file, "rb");

                assert(adef->restart);
                if(adef->grouping)
                {
                    if (! treeReadLenMULT(f2, tr, adef))
                        exit(-1);
                }
                else
                {
                    if(tr->saveMemory)
                        treeReadLen(f2, tr, FALSE, FALSE, TRUE, adef, FALSE, FALSE);
                    else
                        treeReadLen(f2, tr, FALSE, FALSE, FALSE, adef, FALSE, FALSE);
                }
...
void doInference(tree *tr, analdef *adef, rawdata *rdta, cruncheddata *cdta)
{
    ...
    for(int i3 = 0; i3 < num_runs; i3++) {
        unOptLikes[i3] = unlikely;
        optLikes[i3] = unlikely;
    }

#ifdef _WAYNE_MPI
    if(num_runs % processes != 0)
        num_runs = processes * ((num_runs / processes) + 1);
#endif

    if(!tr->catOnly) {
        rl = (topolRELL_LIST *)rax_malloc(sizeof(topolRELL_LIST));
        initTL(rl, tr, num_runs);
    }
    ...
    for(int run_no = 0; run_no < num_runs; run_no++)
    {
        ...
        unOptLikes[run_no] = tr->likelihood;

        if(tr->catOnly) {
            printMesquiteLog(adef, unOptLikes[run_no], 0.0, TRUE);
    ...
    if (tr->catOnly) {
        ...
        if(tr->rateHetModel == GAMMA ||  tr->rateHetModel == GAMMA_I) {
            ...
            for(int i4 = 0; i4 < num_runs; i4++) {
                ...
                if(i4 != best) {
                    ...
                    evaluateGenericInitrav(tr, tr->start);
                    treeEvaluate(tr, 1);
                    tr->likelihoods[i4] = tr->likelihood;
                ...
                printMesquiteLog(adef, unOptLikes[i4], optLikes[i4], FALSE);

                if(num_runs == 1)
                    printBothOpen("Inference[%d] final GAMMA-based Likelihood: %f tree written to file %s\n", i4, tr->likelihoods[i4], resultFileName);
                else
                    printBothOpen("Inference[%d] final GAMMA-based Likelihood: %f tree written to file %s.RUN.%d\n", i4, tr->likelihoods[i4], resultFileName, i4);
#endif
            }
        }
        else {
...
#ifdef _WAYNE_MPI
            for(int i5 = 0; i5 < num_runs; i5++) {
                j = i5 + n*processID;
                rl->t[j]->likelihood = unlikely;
            }
#else
            for(int i6 = 0; i6 < num_runs; i6++)
                rl->t[i6]->likelihood = unlikely;
#endif
            ...
            for(int i7 = 0; i7 < num_runs; i7++) {
                ...
                if(num_runs == 1 && processes == 1)
                    printBothOpen("Inference[%d] final GAMMA-based Likelihood: %f tree written to file %s\n", i7, tr->likelihoods[i7], resultFileName);
                else
                    printBothOpen("Inference[%d] final GAMMA-based Likelihood: %f tree written to file %s.RUN.%d\n", j, tr->likelihoods[j], resultFileName, j);
#else
                if(i7 != best) {
                    ...
                    evaluateGenericInitrav(tr, tr->start);
                    treeEvaluate(tr, 2);
                    tr->likelihoods[i7] = tr->likelihood;
                ...
                printMesquiteLog(adef, unOptLikes[i7], optLikes[i7], FALSE);

                if(num_runs == 1)
                    printBothOpen("Inference[%d] final GAMMA-based Likelihood: %f tree written to file %s\n", i7, tr->likelihoods[i7], resultFileName);
                else
                    printBothOpen("Inference[%d] final GAMMA-based Likelihood: %f tree written to file %s.RUN.%d\n", i7, tr->likelihoods[i7], resultFileName, i7);
#endif
            }
        }
        ...
        if (processes > 1) {
            ...
            for(int i8 = 0; i8 < processes; i8++)
                buffer[i8] = unlikely;
            buffer[processID] = bestLH;
            for(int i9 = 0; i9 < processes; i9++)
                MPI_Bcast(&buffer[i9], 1, MPI_DOUBLE, i9, MPI_COMM_WORLD);
            bestLH = buffer[0];
            bestProcess = 0;
            for(int i10 = 1; i10 < processes; i10++)
                if(buffer[i] > bestLH) {
                    bestLH = buffer[i10];
                    bestProcess = i10;
                }
    ...
    if(adef->rellBootstrap) {
        ...
        for(int i2 = 0; i2 < NUM_RELL_BOOTSTRAPS; i2++) {
