...
void computeBIGRAPID(tree *tr, analdef *adef, boolean estimateModel)
{
    ...
    while (impr) {
        ...
        for (int i = 1; i <= bt->nvalid; i++) {
            ...
            difference = ((tr->likelihood > previousLh) ? tr->likelihood - previousLh : previousLh - tr->likelihood);
        ...
        if (impr && adef->stop_after_seconds > 0 && elapsed > adef->stop_after_seconds) {
            impr = 0;
            printBothOpenMPI("\n\nStopping search in computeBIGRAPID(main): --stop-after-seconds %d and elapsed time %f\n\n", adef->stop_after_seconds, elapsed);
            break;
        }
    }
...
cleanup_fast:
    ...
    while (1) {
        ...
        // printf(">>> computeBIGRAPID cleanup_fast: while(1) end impr:%d [%g] <%f>\n", impr, tr->likelihood, gettime() - masterTime);

        const double elapsed = gettime() - masterTime;
        if (adef->stop_after_seconds > 0 && elapsed > adef->stop_after_seconds) {
            impr = 0;
            rearrangementsMax = adef->max_rearrange + 1;
            printBothOpenMPI("\n\nStopping search in computeBIGRAPID(cleanup_fast): --stop-after-seconds %d and elapsed time %f\n\n", adef->stop_after_seconds, elapsed);
        }
    }
...
cleanup:
...
#ifdef _TERRACES
    {
        const double bestLH = tr->likelihood;
        FILE* f = myfopen(terraceFileName, "w");
        ...
        for (int i = 1; i <= terrace->nvalid; i++) {
    ...
    // printf(">>> computeBIGRAPID cleanup: [%g] <%f>\n", tr->likelihood, gettime() - masterTime);
    if (tr->searchConvergenceCriterion) {
    ...
    printLog(tr, adef, FALSE);
    printResult(tr, adef, FALSE);
    // printf(">>> computeBIGRAPID done: [%g] <%f>\n", tr->likelihood, gettime() - masterTime);
}
...
boolean treeEvaluate (tree *tr, double smoothFactor)       /* Evaluate a user tree */
{
    ...
    boolean result = smoothTree(tr, (int)((double)smoothings * smoothFactor));
    assert(result);
...
{
    const int countBranches = tr->branchCounter;
